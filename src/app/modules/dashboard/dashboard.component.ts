...
interface SalesData {
  ...
  previousSalesAllOutlets: number;
  previousSalesHTSA: number;
  previousSalesHTL: number;
  previousSalesHTG: number;
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  previousSalesAllOutlets: number = 0;
  previousSalesHTSA: number = 0;
  previousSalesHTL: number = 0;
  previousSalesHTG: number = 0;
  ...
  loadSalesData() {
    ...
    this.http.get<SalesData>(apiUrl).subscribe({
      next: (data) => {
        ...
        this.previousSalesAllOutlets = data.previousSalesAllOutlets || 0;
        this.previousSalesHTSA = data.previousSalesHTSA || 0;
        this.previousSalesHTL = data.previousSalesHTL || 0;
        this.previousSalesHTG = data.previousSalesHTG || 0;
  ...
  updateChartData() {
    ...
    const blueColorDark = documentStyle.getPropertyValue('--blue-300');
    ...
    const orangeColorDark = documentStyle.getPropertyValue('--orange-300');
    const cyanColor = documentStyle.getPropertyValue('--cyan-500');
    const cyanColorLight = documentStyle.getPropertyValue('--cyan-100');
    const cyanColorDark = documentStyle.getPropertyValue('--cyan-300');
    ...
    const purpleColorDark = documentStyle.getPropertyValue('--purple-300');
    const grayColor = documentStyle.getPropertyValue('--gray-500');
    const grayColorLight = documentStyle.getPropertyValue('--gray-100');
    ...
    let currentAllOutletsData: number[] = [];
    let previousAllOutletsData: number[] = [];

    if (this.selectedYear) {
      // If a year is selected
      const yearLabel = this.selectedYear.getFullYear().toString();
      const prevYear = (this.selectedYear.getFullYear() - 1).toString();
      // Current period
      currentAllOutletsData = [this.totalSalesAllOutlets, this.totalSalesHTSA, this.totalSalesHTL, this.totalSalesHTG];
      // Previous period
      previousAllOutletsData = [this.previousSalesAllOutlets, this.previousSalesHTSA, this.previousSalesHTL, this.previousSalesHTG];
    } else if (this.selectedMonth) {
      ...
      const prevMonthLabel = prevMonth.toLocaleDateString([], { month: 'long', year: 'numeric' });
      // Current period
      currentAllOutletsData = [this.totalSalesAllOutlets, this.totalSalesHTSA, this.totalSalesHTL, this.totalSalesHTG];
      // Previous period
      previousAllOutletsData = [this.previousSalesAllOutlets, this.previousSalesHTSA, this.previousSalesHTL, this.previousSalesHTG];
      ...
      const dateLabel = this.selectedDate
        ...
        : 'Today';
      const prevDate = new Date(this.selectedDate || new Date());
      prevDate.setDate(prevDate.getDate() - 1);
      const prevDateLabel = prevDate.toLocaleDateString();
      // Current period
      currentAllOutletsData = [this.totalSalesAllOutlets, this.totalSalesHTSA, this.totalSalesHTL, this.totalSalesHTG];
      // Previous period
      previousAllOutletsData = [this.previousSalesAllOutlets, this.previousSalesHTSA, this.previousSalesHTL, this.previousSalesHTG];
    ...
    this.chartData = {
      labels: ['ALL', 'HTSA', 'HTL', 'HTG'],
      datasets: [
        {
          label: 'Previous',
          data: previousAllOutletsData,
